...
void master(int size) {
    ...
    for(src = 0; src < size; src++) {
        if(src != SERVER_RANK) {
            MPI_Recv(message, max_message_length, MPI_CHAR, src, MESSAGE_TAG, MPI_COMM_WORLD, &stat);
...
void slave(rank) {
    ...
    MPI_Send(message, message_length, MPI_CHAR, SERVER_RANK, MESSAGE_TAG, MPI_COMM_WORLD);
}